...
_DEFAULT_SF_PROMPT = (
    ...
    "日期与金额规则：\n"
    "6) document_type=invoice 时，invoice_date 取“开票日期”，payment_date 返回 null；amount 取价税合计/小写总额。\n"
    "7) document_type=payment_proof 时，payment_date 取“支付时间/转账时间/交易时间”，invoice_date 返回 null；amount 取实际支付金额，负号不影响金额本身。\n"
    "8) 日期只能取票据上出现的明确日期；未找到则返回 null，不要猜测或使用固定日期。\n"
    "9) 类别只能在以下映射中选择，如无匹配填\"其他\"：\n"
