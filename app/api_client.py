...
_KEYWORD_DATE_PATTERN = re.compile(
    r"(?P<keyword>开票日期|支付时间|付款时间|交易时间|转账时间)[：:：\s]*(?P<year>19\d{2}|20\d{2})[年/-](?P<month>1[0-2]|0?[1-9])[月/-](?P<day>3[01]|[12][0-9]|0?[1-9])",
)
_KEYWORD_PRIORITY = {
    "开票日期": 0,
    "支付时间": 1,
...
_DEFAULT_SF_PROMPT = (
    ...
    "2) 判为 payment_proof 的强特征：手机支付详情截图，出现“支付成功”“当前状态 支付成功”“付款成功”“转账成功”“交易成功”"
    "“账单详情”“全部账单”“淘宝账单”“支付时间”“转账时间”“支付方式”“付款方式”“交易单号”“交易订单号”“转账单号”“商户单号”“商家订单号”“收单机构”“银行卡”“零钱”"
    ...
    "7) document_type=payment_proof 时，payment_date 取“支付时间/转账时间/交易时间”，invoice_date 返回 null；amount 只能取实际支付金额，负号不影响金额本身；支付宝/淘宝账单详情页顶部大号负数（如 -180.00）就是实际支付金额。\n"
    ...
    "   - 五金：五金、劳保、劳保用品、工具、配件、耗材、塑料制品、塑胶制品、塑料件、电瓶、电池、铁锹、扳手、螺丝、螺栓、螺母、钳、刀、砂轮、焊机、焊条、安全帽、手套、口罩、hardware\n"
    "请只输出JSON，不要包含解释。"
)
